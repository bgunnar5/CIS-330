              ...
              int* color, int* distance)
{
    ...
    // Do BFS until done
    while(!done) {
	// Initialize a variable to tell us if we've found any neighbors
	int neighbor_found = 0;

	// Reset the dst vector to be all zeroes
	reset_vector(dst, cols);

        // given a vector of source vetices, find the neighbors
	for (int i = 0; i < cols; i++) {
	    // Only go into the next loop if the source vertex exists
	    if (src[i] == 1) {
	    	for (int j = 0; j < rows; j++) {
		    // Determine if there's a neighbor for the source vertex
		    int neighbor = mat_trans[j][i] * src[i];
		    // If there is, set the neighbor's index to 1 in the dst list
		    if (neighbor) {
			neighbor_found = 1;
			dst[j] = 1;
		    }
	...
	// Update the src list for the next iteration to be the neighbors we found previously
	for (int i = 0; i < rows; i++) {
	    int temporary = dst[i];
	    src[i] = temporary;
	}

        // Before we begin, eliminate vertices that have already been visited from the transpose matrix
        for (int i = 0; i < rows; i++) {
	    if (color[i] == 2) {
	    	for (int j = 0; j < cols; j++) {
			mat_trans[i][j] = 0;
