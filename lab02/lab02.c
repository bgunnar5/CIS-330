...
{
	// if statement to capture just the first column
	if (col % cols == 0) {
		// get the length of the string to convert to uppercase
...
apply_by_col(void (*func)(char*, int, int, int, int),
             ...
             int rows, int cols)
{
  // same logic as apply_by_row just with some variables flipped around
  int i, j;
  for (i = 0; i < cols; i++)
  {
    for (j = 0; j < rows; j++)
