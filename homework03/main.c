...
{
	// Allocate memory for the arrays produced by this function
	...
	*csr_vals = (double*)malloc(nnz * sizeof(double));
	...
	// Create a histogram of size m since we're counting rows
	unsigned int hist[m + 1];
	
	// Initialize all indices of the histogram to be 0
	for (int i = 0; i < m + 1; i++) {
		hist[i] = 0;
	}
	
	// Populate the buckets with the amount of times a row appears
	for (int i = 0; i < nnz; i++) {
		hist[row_ind[i] - 1] += 1;
	}
	
	// Calculate the values for each index of csr_row_ptr and store them
	for (int i = 0; i < m + 1; i++) {
		// Start at index 1
		if (i == 0) {
			(*csr_row_ptr)[i] = 1;
	...
	// Build the csr_col_ind array and the csr_vals array
	for (int i = 0; i < nnz; i++) {
		// Obtain the index value to use
